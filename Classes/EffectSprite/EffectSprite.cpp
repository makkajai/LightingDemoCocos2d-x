...
namespace effect {
    ...
    EffectSprite *EffectSprite::createWithSpriteFrameName(const std::string &filename) {
        auto ret = new(std::nothrow) EffectSprite;
        if (ret && ret->initWithSpriteFrameName(filename)) {
            ret->autorelease();
            return ret;
        }
        CC_SAFE_RELEASE(ret);
    ...
    void EffectSprite::initLampWithSpriteFrameName(const std::string &normalMapFilename, Color3B darkValue) {
        ...
        if (_effectHolder == nullptr) {
            ...
            SpriteFrame *pFrame = SpriteFrameCache::getInstance()->getSpriteFrameByName(normalMapFilename);
            _normalmap = pFrame->getTexture();
